...
from robot_ik.collision import (
    CollisionChecker, Sphere, Capsule, Box,
    CollisionResult, ContactPoint
)
    ...
    def _build_collision_model(self):
        ...
        for i in range(6):
            self.checker.add_shape(
                f"arm1_link{i}",
                Capsule(length=self.link_lengths[i], radius=self.link_radius)
            )
        ...
        if check_self:
            for i in range(6):
                for j in range(i + 2, 6):  # Skip adjacent
                    collision = self.checker.check_pair_collision(
                        f"arm1_link{i}", f"arm1_link{j}"
                    )
...
def visualize_collision(q1, q2, collision_result, arm_offset, title="Collision Detection"):
    ...
    ax.scatter(
        [fk1[0, 3]], [fk1[1, 3]], [fk1[2, 3]],
        c='blue', marker='o', s=100, label='Arm1 EE'
    )
    ax.scatter(
        [fk2[0, 3]], [fk2[1, 3]], [fk2[2, 3]],
        c='red', marker='o', s=100, label='Arm2 EE'
    )
    
    # Plot base positions
    ax.scatter([0], [0], [0], c='blue', marker='^', s=200, label='Arm1 Base')
    ax.scatter([arm_offset[0]], [arm_offset[1]], [arm_offset[2]], 
               c='red', marker='^', s=200, label='Arm2 Base')
    
    # Plot collision points
    if collision_result.collided:
        collision_points = np.array([
            [c.position[0], c.position[1], c.position[2]]
            for c in collision_result.contacts
        ])
        ax.scatter(
            collision_points[:, 0],
            collision_points[:, 1],
