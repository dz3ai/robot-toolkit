...
class CoordinatedTrajectory:
    ...
    def __init__(self, traj1, traj2, times):
        ...
        assert len(traj1) == len(traj2) == len(times), "Trajectories must have same length"
...
def generate_pick_and_place_trajectory(
    pick_pose, place_pose, gripper_offset, duration=3.0, n_waypoints=10
):
    """
    ...
    traj1 = waypoint_trajectory(traj1_waypoints, times, profile_type="s-curve")

    # Arm2 trajectory (synchronized, with gripper offset)
    traj2_waypoints = [wp + gripper_offset for wp in waypoints]
    traj2 = waypoint_trajectory(traj2_waypoints, times, profile_type="s-curve")
...
def visualize_coordinated_trajectory(traj, arm_offset, title="Coordinated Dual-Arm Trajectory"):
    """
    ...
    # Plot trajectories
    ax1.plot(ee_pos1[:, 0], ee_pos1[:, 1], ee_pos1[:, 2], "b-", linewidth=2, label="Arm1 Path")
    ax1.plot(ee_pos2[:, 0], ee_pos2[:, 1], ee_pos2[:, 2], "r-", linewidth=2, label="Arm2 Path")

    # Plot start and end
    ax1.scatter(
        ee_pos1[0, 0], ee_pos1[0, 1], ee_pos1[0, 2], c="green", marker="o", s=100, label="Start"
    )
    ax1.scatter(
        ee_pos1[-1, 0], ee_pos1[-1, 1], ee_pos1[-1, 2], c="orange", marker="X", s=100, label="End"
    )
    ...
    for i in range(6):
        ax2.plot(traj.times, traj.traj1[:, i], label=f"J{i+1}", alpha=0.7)
    ...
    for i in range(6):
        ax3.plot(traj.times[:-1], vel1[:, i], label=f"J{i+1}", alpha=0.7)
...
def main():
    ...
    traj = generate_pick_and_place_trajectory(
        pick_pose, place_pose, gripper_offset, duration=3.0, n_waypoints=8
    )
    ...
    fig = visualize_coordinated_trajectory(
        traj, np.array([1.5, 0, 0]), title="Coordinated Pick-and-Place (3s)"
    )
