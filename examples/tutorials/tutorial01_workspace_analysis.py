...
def sample_workspace(robot_model, n_samples=2000, joint_limits=None):
    ...
    for _ in range(n_samples):
        ...
        q = np.random.uniform(
            low=[l[0] for l in joint_limits],
            high=[l[1] for l in joint_limits],
            size=6
        )
...
def visualize_dual_workspace(points1, points2, arm_offset, title="Dual-Arm Workspace"):
    ...
    # Arm1 workspace (blue)
    ax.scatter(
        points1[idx1, 0], points1[idx1, 1], points1[idx1, 2],
        c='blue', alpha=0.15, s=10, label='Arm1 Workspace'
    )
    
    # Arm2 workspace (red)
    ax.scatter(
        points2[idx2, 0], points2[idx2, 1], points2[idx2, 2],
        c='red', alpha=0.15, s=10, label='Arm2 Workspace'
    )
...
def main():
    ...
    fig = visualize_dual_workspace(
        points1, points2, arm2_offset,
        title="Dual-Arm Workspace (1.5m Offset)"
    )
