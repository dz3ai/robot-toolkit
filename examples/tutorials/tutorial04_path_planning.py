...
class ObstacleEnvironment:
    ...
    def _add_default_obstacles(self):
        ...
        self.checker.add_shape(
            "column",
            Box(size=[0.2, 0.2, 1.0], position=[0.5, 0.0, 0.5])
        )
        
        # Side obstacle
        self.checker.add_shape(
            "side_block",
            Box(size=[0.3, 0.3, 0.3], position=[0.7, 0.3, 0.4])
        )
    ...
    def is_collision_free(self, q):
        ...
        for shape_name in ["column", "side_block"]:
            collision = self.checker.check_point_collision(
                ee_pos, shape_name
            )
...
def visualize_rrt_result(
    ...
    if path is not None:
        ...
        # Plot start and goal
        ax1.scatter(
            [ee_positions[0, 0]], [ee_positions[0, 1]], [ee_positions[0, 2]],
            c='green', marker='o', s=100, label='Start'
        )
        ax1.scatter(
            [ee_positions[-1, 0]], [ee_positions[-1, 1]], [ee_positions[-1, 2]],
...
def plan_dual_arm_sequential():
    ...
    if path1 is not None:
        ...
        env.checker.add_shape(
            "arm1_final",
            Sphere(radius=0.15, position=ee1_pos)
        )
    else:
...
def main():
    ...
    fig = visualize_rrt_result(
        path, env, start, goal,
        title="RRT* Path Planning with Obstacles"
    )
