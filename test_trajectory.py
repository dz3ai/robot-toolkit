...
def test_quintic_boundary_conditions():
    ...
    traj = joint_quintic_interpolation(q_start, q_end, duration, 
                                       v_start=v_start, v_end=v_end, dt=0.01)
    ...
    assert np.allclose(traj.joint_velocities[0], v_start, atol=1e-3), \
        f"Start velocity mismatch: {traj.joint_velocities[0]} vs {v_start}"
    assert np.allclose(traj.joint_velocities[-1], v_end, atol=1e-3), \
...
def test_cartesian_straight_line():
    ...
    for idx in indices:
        ...
        assert deviation < 0.001, f"Deviation {deviation*1000:.2f}mm > 1mm at t={traj.time_points[idx]:.2f}"
...
def test_s_curve_jerk_continuity():
    ...
    assert max_accel_jump < threshold, \
...
def test_waypoint_no_blend():
    ...
    for i, (wp, t_target) in enumerate(zip(waypoints, times)):
        ...
        assert np.allclose(q_actual, wp, atol=1e-3), \
